...
from Reze import db as xdb
from Reze import telethn
from Reze.custom_filter import callbackquery, register
from Reze.functions.admins import (
    can_ban_users,
    cb_can_ban_users,
    extract_time,
    get_time,
...
from Reze.utils.decorators import exception, logging
...
async def dnd(e):
    ...
    if not q:
        if not x:
            ...
            await e.reply(
                "**DND** mode is currently on, Reze will autokick newly joined users without usernames."
            )
