...
from Reze import API_HASH, API_ID, LOGGER, db, CLONE_LIMIT, SUPPORT_CHAT
from Reze.custom_filter import register, auth
from Reze.tele.clone_manager import clone_manager
...
async def clone_bot(event):
    ...
    if not event.is_private:
        return await event.reply("Please clone **Reze** in your private chat.")
    
    user_id = event.sender_id
    if await clone_db.find_one({"_id": user_id}):
        return await event.reply(
            "You have already cloned **Reze**. If you want to delete the clone, use `/deleteclone <bottoken>`"
        )
    
    if len(event.text.split()) == 1:
        return await event.reply(
            "Please provide the bot token from @BotFather in order to clone **Reze**.\n**Example**: `/clone 219218219:jksswq`"
        )
    ...
    if await clone_db.find_one({"token": token}):
        return await event.reply("The same bot token has been used to clone **Reze**. Please use a different bot token.")
    ...
    try:
        ...
        await wait.delete()
        await event.reply("An error occurred while cloning **Reze**. Please try again or contact support @SpiralTechDivision.")
...
async def delete_cloned(event):
    ...
    if not event.is_private:
        return await event.reply("Please delete Reze's clone in your private chat.")
    
    user_id = event.sender_id
    check = await clone_db.find_one({"_id": user_id})
    if not check:
        return await event.reply(
            "You have not cloned **Reze** yet. If you want to clone it, use `/clone <bottoken>`"
        )
    
    if len(event.text.split()) == 1:
        return await event.reply(
            "Please provide the bot token from @BotFather in order to delete the cloned **Reze**. Example: `/deleteclone 219218219:jksswq`"
        )
...
async def set_startpic(event):
    if not getattr(event.client, "is_clone", False):
        return await event.reply("This feature is only available in cloned bots. Learn more about cloning Reze by using `/help Clone`.")
